  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#0B0B0B' }}>
      ...
      <View pointerEvents="none" style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, overflow: 'hidden' }}>
        <View style={{ position: 'absolute', top: -100, alignSelf: 'center', width: 400, height: 260, borderRadius: 200, backgroundColor: 'rgba(246,211,101,0.05)' }} />
      </View>
      ...
      <View style={{ paddingHorizontal: 20, paddingTop: 8, paddingBottom: 4, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        <Pressable
          ...
          <FontAwesome name="bars" size={13} color="#F5F5F5" />
        </Pressable>
        <View style={{ flexDirection: 'row' }}>
          <Text style={{ color: '#F5F5F5', fontSize: 18, fontWeight: '700' }}>Chat</Text>
          <Text style={{ color: '#F6D365', fontSize: 18, fontWeight: '700' }}>Line</Text>
        </View>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 40 }}>
        ...
        <Animated.View style={{ marginTop: 32, opacity: badgeAnim, alignSelf: 'flex-start' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, borderRadius: 999, borderWidth: 1, borderColor: 'rgba(246,211,101,0.20)', backgroundColor: 'rgba(246,211,101,0.06)', paddingHorizontal: 14, paddingVertical: 7 }}>
            <Text style={{ color: '#F6D365', fontSize: 13 }}>✦</Text>
        ...
        {/* ─── Subtitle ── */}
        <Animated.Text style={{ marginTop: 14, color: '#A1A1A1', fontSize: 14, lineHeight: 22, opacity: subtitleAnim }}>
          {locale === 'fr'
        ...
        <View style={{ marginTop: 32, flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
          {FEATURES.map((f, i) => (
            <Animated.View
              ...
              <View style={{ borderRadius: 18, borderWidth: 1, borderColor: 'rgba(255,255,255,0.07)', backgroundColor: '#111111', padding: 18, minHeight: 110 }}>
                <Text style={{ fontSize: 22, marginBottom: 10 }}>{f.icon}</Text>
                <Text style={{ color: '#F5F5F5', fontSize: 14, fontWeight: '600' }}>{f.title}</Text>
                <Text style={{ marginTop: 4, color: '#555555', fontSize: 12, lineHeight: 18 }}>{f.desc}</Text>
              </View>
            </Animated.View>
          ))}
        </View>
        ...
        {/* ─── Pricing cards ── */}
        <Text style={{ marginTop: 36, color: '#A1A1A1', fontSize: 10, fontWeight: '600', letterSpacing: 2, marginBottom: 14 }}>
        ...
        <View style={{ flexDirection: 'row', gap: 10 }}>
          ...
          <Animated.View style={{ flex: 1, opacity: pricingAnims[0], transform: [{ translateY: pricingAnims[0].interpolate({ inputRange: [0, 1], outputRange: [20, 0] }) }] }}>
            <View style={{ borderRadius: 20, borderWidth: 1, borderColor: 'rgba(255,255,255,0.07)', backgroundColor: '#111111', padding: 18 }}>
              <Text style={{ color: '#A1A1A1', fontSize: 11, fontWeight: '600', letterSpacing: 1.5 }}>GRATUIT</Text>
              <Text style={{ marginTop: 8, color: '#F5F5F5', fontSize: 28, fontWeight: '800' }}>0€</Text>
          ...
          <Animated.View style={{ flex: 1, opacity: pricingAnims[1], transform: [{ translateY: pricingAnims[1].interpolate({ inputRange: [0, 1], outputRange: [20, 0] }) }] }}>
            <View style={{ borderRadius: 20, borderWidth: 1, borderColor: 'rgba(246,211,101,0.30)', backgroundColor: 'rgba(246,211,101,0.05)', padding: 18 }}>
              <Text style={{ color: '#F6D365', fontSize: 11, fontWeight: '600', letterSpacing: 1.5 }}>PREMIUM</Text>
              <View style={{ marginTop: 8, flexDirection: 'row', alignItems: 'baseline', gap: 2 }}>
                <Text style={{ color: '#F5F5F5', fontSize: 28, fontWeight: '800' }}>8,99€</Text>
                <Text style={{ color: '#A1A1A1', fontSize: 12 }}>/mois</Text>
              </View>
              <View style={{ marginTop: 14, gap: 8 }}>
        ...
        <View style={{ marginTop: 28 }}>
          <Pressable
            ...
            disabled={isPremium}
            style={{ borderRadius: 14, backgroundColor: isPremium ? 'rgba(246,211,101,0.20)' : '#F6D365', paddingVertical: 16, alignItems: 'center' }}>
            <Text style={{ color: isPremium ? '#F6D365' : '#0B0B0B', fontSize: 15, fontWeight: '700' }}>
              {isPremium
                ? (locale === 'fr' ? '✦ Vous êtes Premium' : '✦ You are Premium')
                : (locale === 'fr' ? 'Commencer — 8,99€ / mois' : 'Get started — €8.99 / month')}
            </Text>
          </Pressable>
          <Pressable
...
function FreeLine({ text }: { text: string }) {
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
      ...
      <Text style={{ color: '#A1A1A1', fontSize: 12, flex: 1 }}>{text}</Text>
    </View>
  );
}
...
function PremLine({ text }: { text: string }) {
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
      <Text style={{ color: '#F6D365', fontSize: 12 }}>✓</Text>
      <Text style={{ color: '#F5F5F5', fontSize: 12, flex: 1 }}>{text}</Text>
    </View>
  );
}
